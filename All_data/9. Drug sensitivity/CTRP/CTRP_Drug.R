...
exp[1:4,1:4]
dim(exp) #51847个基因，829个细胞系
exp <- log2(exp+1)

drug = readRDS(file = file.path(dir,"CTRP2_Res.rds")) #829细胞系，545个药物
dim(drug)
#排除含NA值超20%的化合物
#计算每列NA的数量
...
#这里我选择的是log2FC<-0.3(high组的AUC值应该选更低的), 筛选得到27个药物
...
cor_drug <- cor_result$Drug  #29个

#最终得到14种药物
inter_drug <- intersect(df_drug,cor_drug)
write.csv(inter_drug,file = "./CTRP_inter_drug.csv")
...
pdf(file = "./CTRP_boxplot.pdf",height = 6,width = 12)
...
pdf("./CTRP_cor.pdf",width = 10,height = 6)
