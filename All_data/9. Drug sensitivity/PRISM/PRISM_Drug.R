...
exp = read.csv('Expression_Public_23Q4.csv',row.names = 1,check.names = F)
exp <- t(exp)
exp[1:4,1:4]
dim(exp) #19144个基因，1479个细胞系


drug = read.csv('Drug_sensitivity_(PRISM_Repurposing_Primary_Screen).csv',row.names = 1,check.names = F) 

dim(drug)#568细胞系,4659个药物

#取交集细胞系
cell <- intersect(colnames(exp),rownames(drug))
...
drug4[1:4,1:4]
identical(rownames(drug4),colnames(exp))
...
#这里我选择的是log2FC<-0.3(high组的AUC值应该选更低的), 筛选得到10个药物
...
cor_drug <- cor_result$Drug  #358个

#最终得到9种药物
inter_drug <- intersect(df_drug,cor_drug)
write.csv(inter_drug,file = "./PRISM_inter_drug.csv")
...
dat1$Drug <-gsub("\\s*\\(.*?\\)", "", dat1$Drug)

library(stringr)
dat1$Drug <- str_to_title(dat1$Drug) #将第一个字母转换为大写

a <- dfData3[dfData3$Drug%in%inter_drug,]
b <- a[match(sort(a$Drug),a$Drug),]

pdf(file = "./PRISM_boxplot.pdf",height = 6,width = 12)
...
dat2$Drug <-gsub("\\s*\\(.*?\\)", "", dat2$Drug)
dat2$Drug <- str_to_title(dat2$Drug)
dat2 <- dat2[order(dat2$r),]
dat2$Drug <- factor(dat2$Drug,levels = rev(dat2$Drug))
dat2$P  <- -log10(dat2$p.value) #Inf 变为20
dat2$P[1:3] <- c(20,20,20)


pdf("./PRISM_cor.pdf",width = 10,height = 6)
